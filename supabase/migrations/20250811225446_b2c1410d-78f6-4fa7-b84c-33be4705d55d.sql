...
DECLARE
  ...
  v_welcome_id UUID;
BEGIN
  -- 找到 Elena 的 therapist_id（generation_prompts.therapist_id 是 TEXT）
  ...
  -- 创建 Daily Message persona 提示（prompt_type='text', language='en'）
...
End with a soft invitation to share or connect$$,
  ...
  -- 将同类型/语言的其它旧记录设为 inactive（避免 .single() 冲突）
  ...
  -- 创建 Welcome 提示（prompt_type='welcome', language='en'）
  INSERT INTO public.generation_prompts (
    therapist_id, therapist_name, prompt_type, language, prompt_text, active
  )
  VALUES (
