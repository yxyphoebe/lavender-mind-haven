...
DECLARE
  v_tid TEXT;
BEGIN
  -- 1) 拿到 Elena 的 therapist_id（作为 TEXT 存入 generation_prompts）
  ...
  -- 2) 更新/插入 Daily Message persona 提示（prompt_type='text', language='en'）
  -- 2a) 若已有 active 版本则更新文本
...
End with a soft invitation to share or connect$$,
  ...
  IF NOT FOUND THEN
    -- 2b) 将其它同类型同语言的 prompt 设为 inactive（避免多条 active）
    UPDATE public.generation_prompts
    SET active = false
    WHERE therapist_id = v_tid
      AND prompt_type = 'text'
...
End with a soft invitation to share or connect$$,
  ...
  -- 3) 更新/插入 Welcome 提示（prompt_type='welcome', language='en'）
  -- 3a) 若已有 active 版本则更新文本
  UPDATE public.generation_prompts
  SET
    therapist_name = 'Elena',
  ...
  IF NOT FOUND THEN
    -- 3b) 将其它同类型同语言的 prompt 设为 inactive
    UPDATE public.generation_prompts
    SET active = false
    WHERE therapist_id = v_tid
      AND prompt_type = 'welcome'
